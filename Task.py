...
class Task:
	...
	def __init__(self, referred_id = 'Nan', issue_desc = "", admin_comment = "", date_resolved = "", resolved = False):
		self.id = 'Nan'
		#might call new_issue later on
		self.new_issue(referred_id, issue_desc, admin_comment, date_resolved, resolved)
		
	#create a new issue in db and in class
	def new_issue(self, referred_id, issue_desc, admin_comment = "", date_resolved = "", resolved = False): 
		self.set_all(referred_id, issue_desc, admin_comment, date_resolved, resolved)
	...
	def set_all(self, referred_id, issue_desc, admin_comment, date_resolved, resolved, modify_db = 0):
	...
	def dump(self,dict):
		self.set_all(dict["referred_id"], dict["issue_desc"], dict["admin_comment"], dict["date_resolved"], dict["resolved"])
	...
	def get_all(self):
		...
		"admin_comment":self.admin_comment, "date_resolved":self.date_resolved, "resolved":self.resolved}
