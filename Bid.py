...
class Bid_db:
	...
	def new_bid(self, client_id, project_id, start_date, end_date, bid_log, status):
		self.bid_id = jsonIO.get_last_id(self.db) + 1 #last+1 for new
		jsonIO.add_row(self.db, self.bid_id, client_id, project_id, start_date, end_date, bid_log)
	...
	def load_db(self, bid_id):
		if bid_id != 'Nan':
			self.bid_id = bid_id
			self.dump(jsonIO.load_db(self.db, bid_id))
			return 1
		else:
			return 0
	...
	def set_bid_id(self, bid_id):
		jsonIO.set_(self.db, self.bid_id, "id", bid_id)
		self.bid_id = bid_id
		return 1;
	def set_client_id(self, client_id):
		self.client_id = client_id
		jsonIO.set_(self.db, self.bid_id, "client_id", client_id)
		return 1;
	def set_project_id(self, project_id):
		self.project_id = project_id
		jsonIO.set_(self.db, self.bid_id, "project_id", project_id)
		return 1;
	def set_start_date(self, start_date):
		self.start_date = start_date
		jsonIO.set_(self.db, self.bid_id, "start_date", start_date)
		return 1;
	def set_end_date(self, end_date):
		self.end_date = end_date
		jsonIO.set_(self.db, self.bid_id, "end_date", end_date)
		return 1;
	def set_bid_log(self, bid_log):
		self.bid_log = bid_log[:]
		jsonIO.set_(self.db, self.bid_id, "bid_log", bid_log)
		return 1;
	def set_status(self, status):
		self.status = status
		jsonIO.set_(self.db, self.bid_id, "status", status)
		return 1;

#destructor
	def remove(self):
		jsonIO.del_row(self.db, self.bid_id)
